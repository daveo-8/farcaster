# race_with_countdown_stop_fix.py
import math
import pygame
...
FINISH_WIDTH_PX   = 6
FINISH_DIRECTION  = 'left'   # 'left' 从右向左；'right' 从左向右
...
class HorseSprite:
    ...
    vx: 当前水平速度（屏幕坐标，>0 向右）
    target_vx: 目标水平速度
    accel: 每秒速度变化量（朝 target_vx 逼近）
    plan: [{'t': 秒, 'target_vx': 速度, 'accel': 加速度}, ...]
    """
    ...
    def update(self, dt, position_enabled=True):
        # 切帧（始终可播放；若倒计时也要静止，可把这几行挪到 if position_enabled）
        self.acc_anim += dt
    ...
    while running:
        ...
        while acc >= FIXED_DT:
            if state == STATE_COUNTDOWN:
                # 仅推进倒计时计时器；不更新背景、不更新马、不更新红线
                countdown_t += FIXED_DT

                # 如果你之前在倒计时里调用过这些，统统删掉 / 注释掉：
                # far.update(FIXED_DT)
                # near.update(FIXED_DT)
                # for h in horses: h.update(FIXED_DT, position_enabled=False)
                # if USE_FINISH_LINE: finish.update(FIXED_DT, time_enabled=False)
